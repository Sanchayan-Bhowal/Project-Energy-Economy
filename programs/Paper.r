Data <- read.csv("data/Data.csv") # nolint
...
lines(1:13, hedonic_price1[-1], type = "o", col = "#3C78D8", lwd = 3)
lines(1:13, hedonic_price2[-1], type = "o", col = "#8EC400", lwd = 3)
axis(2)
axis(1,
    at = 1:13,
...
windows()
tp1 <- ggplot(data = Data, aes(ln_price_1)) +
    geom_histogram(aes(y = ..count.. / sum(..count..)),
        bins = 30, col = border, fill = fill
    ) +
    ylab("Fraction") +
    ggtitle("Transformed first transaction price")

tp2 <- ggplot(data = Data, aes(t_price_2)) +
    geom_histogram(aes(y = ..count.. / sum(..count..)),
        bins = 30, col = border, fill = fill
    ) +
    ylab("Fraction") +
    ggtitle("Transformed second transaction price")

grid.arrange(tp1, tp2, nrow = 1, ncol = 2)

windows()
p1 <- ggplot(data = Data, aes(price_1)) +
    geom_histogram(aes(y = ..count.. / sum(..count..)),
        bins = 30, col = border, fill = fill
    ) +
    ylab("Fraction") +
    ggtitle("First transaction price")

p2 <- ggplot(data = Data, aes(price_2)) +
    geom_histogram(aes(y = ..count.. / sum(..count..)),
        bins = 30, col = border, fill = fill
    ) +
    ylab("Fraction") +
    ggtitle("Second transaction price")
